...
public class InventarioController {
    ...
    @Autowired
    private IInventario inventarioService; // Inyección de dependencia del servicio de cliente
    ...
    public Inventario create(@RequestBody Inventario inventario){
        return inventarioService.save(inventario); // Guarda un nuevo cliente y retorna el cliente creado
    }
    ...
    public Inventario update(@RequestBody Inventario inventario){
        return inventarioService.save(inventario); // Actualiza un cliente existente y retorna el cliente actualizado
    }
    ...
    public Inventario showById(@PathVariable Integer id){
        return inventarioService.findById(id); // Retorna un cliente específico por su ID con el código de estado HTTP 200 (OK)
    }

    // Maneja solicitudes GET a la URL "/api/v1/clientes"
    @GetMapping("inventarios")
    @ResponseStatus(HttpStatus.OK)
    public @ResponseBody ArrayList<Inventario> showAll(){
        return inventarioService.findByAll(); // Retorna todos los clientes con el código de estado HTTP 200 (OK)
    }
