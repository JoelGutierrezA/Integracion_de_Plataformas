...
public class VentaController {
    ...
    @Autowired
    private IVenta ventaService; // Inyección de dependencia del servicio de cliente
    ...
    public Venta create(@RequestBody Venta venta){
        return ventaService.save(venta); // Guarda un nuevo cliente y retorna el cliente creado
    }
    ...
    public Venta update(@RequestBody Venta venta){
        return ventaService.save(venta); // Actualiza un cliente existente y retorna el cliente actualizado
    }
    ...
    public Venta showById(@PathVariable Integer id){
        return ventaService.findById(id); // Retorna un cliente específico por su ID con el código de estado HTTP 200 (OK)
    }

    // Maneja solicitudes GET a la URL "/api/v1/clientes"
    @GetMapping("ventas")
    @ResponseStatus(HttpStatus.OK)
    public @ResponseBody ArrayList<Venta> showAll(){
        return ventaService.findByAll(); // Retorna todos los clientes con el código de estado HTTP 200 (OK)
    }
