...
public class AdministradorController {
    ...
    @Autowired
    private IAdministrador administradorService; // Inyección de dependencia del servicio de cliente
    ...
    public Administrador create(@RequestBody Administrador administrador){
        return administradorService.save(administrador); // Guarda un nuevo cliente y retorna el cliente creado
    }
    ...
    public Administrador update(@RequestBody Administrador administrador){
        return administradorService.save(administrador); // Actualiza un cliente existente y retorna el cliente actualizado
    }
    ...
    public Administrador showById(@PathVariable Integer id){
        return administradorService.findById(id); // Retorna un cliente específico por su ID con el código de estado HTTP 200 (OK)
    }

    // Maneja solicitudes GET a la URL "/api/v1/clientes"
    @GetMapping("administradores")
    @ResponseStatus(HttpStatus.OK)
    public @ResponseBody ArrayList<Administrador> showAll(){
        return administradorService.findByAll(); // Retorna todos los clientes con el código de estado HTTP 200 (OK)
    }
