...
public class MensajeController {
    ...
    @Autowired
    private IMensaje mensajeService; // Inyección de dependencia del servicio de cliente
    ...
    public Mensaje create(@RequestBody Mensaje mensaje){
        return mensajeService.save(mensaje); // Guarda un nuevo cliente y retorna el cliente creado
    }
    ...
    public Mensaje update(@RequestBody Mensaje mensaje){
        return mensajeService.save(mensaje); // Actualiza un cliente existente y retorna el cliente actualizado
    }
    ...
    public Mensaje showById(@PathVariable Integer id){
        return mensajeService.findById(id); // Retorna un cliente específico por su ID con el código de estado HTTP 200 (OK)
    }

    // Maneja solicitudes GET a la URL "/api/v1/clientes"
    @GetMapping("mensajes")
    @ResponseStatus(HttpStatus.OK)
    public @ResponseBody ArrayList<Mensaje> showAll(){
        return mensajeService.findByAll(); // Retorna todos los clientes con el código de estado HTTP 200 (OK)
    }
