...
public class DetalleController {
    ...
    @Autowired
    private IDetalle detalleService; // Inyección de dependencia del servicio de cliente

    // Maneja solicitudes POST a la URL "/api/v1/cliente"
    @PostMapping("detalle")
    @ResponseStatus(HttpStatus.CREATED)
    public Detalle create(@RequestBody Detalle detalle){
        return detalleService.save(detalle); // Guarda un nuevo cliente y retorna el cliente creado
    }
    ...
    public Detalle update(@RequestBody Detalle detalle){
        return detalleService.save(detalle); // Actualiza un cliente existente y retorna el cliente actualizado
    }
    ...
    public Detalle showById(@PathVariable Integer id){
        return detalleService.findById(id); // Retorna un cliente específico por su ID con el código de estado HTTP 200 (OK)
    }

    // Maneja solicitudes GET a la URL "/api/v1/clientes"
    @GetMapping("detalles")
    @ResponseStatus(HttpStatus.OK)
    public @ResponseBody ArrayList<Detalle> showAll(){
        return detalleService.findByAll(); // Retorna todos los clientes con el código de estado HTTP 200 (OK)
    }
